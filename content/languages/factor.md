...
One of Factor's greatest strengths is its ability to factor words into smaller words. Due to the nature of concatenative programming, this is typically a cut and paste job that can be done almost anywhere there is whitespace. Factor also has impressive metaprogramming capabilities. Since Factor is almost entirely written in Factor, there is full introspection support, including seamless access to Factor's parser, allowing one to define new syntax. Factor also offers Lisp-style macros, and in general, Factor code can be treated like a collection ([homoiconicity](https://en.wikipedia.org/wiki/Homoiconicity)).

## Links
*[Factor programming language](http://factorcode.org)
*[Planet Factor](http://planet.factorcode.org)
*[Factor on concatenative.org](https://concatenative.org/wiki/view/Factor)
