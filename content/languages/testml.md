...
[TestML](http://testml.org) is an [Acmeist](http://acmeism.org), data-driven, unit test language/framework. This means that you can write the same software (since it passes the same tests) in multiple programming languages.
...
TestML tests/programs are typically integrated into the testing framework of a given project. However TestML can also be run as a standalone programming language. To install the TestML runner, install this [Ruby gem](https://rubygems.org/gems/testml-command).
