...
:I'm a little late with this, I suppose, but I've been dinking around with [Piet](http://www.dangermouse.net/esoteric/piet.html) ([Esolangs entry](https://esolangs.org/wiki/Piet)) over the past few days, and its programs are ''entirely'' images. (From the Piet page: "Piet is a programming language in which programs look like abstract paintings.") I like the language (as a toy), but it's possible (although somewhat unlikely) that the images could get... '''big'''. (Biggest one known ''so far'': [Image:http://www.dangermouse.net/esoteric/piet/pietquest.png])
...
::People have reported the same problem across multiple browsers. I've had it myself on Firefox and IE. I found a [really weird workaround](https://irclog.perlgeek.de/rosettacode/2011-07-06#i_4069398). We're hoping that a MediaWiki update can fix it. --[[User:Mwn3d|Mwn3d]] 13:52, 29 November 2011 (UTC)
...
:Just as an example: The Piet program for the Integer Sequence task [Rosettacode link:https://rosettacode.org/wiki/Integer_sequence] needs only 180 bytes at a codel size of 20. The image itself is only 140x60 pixels (7x3 codels) large. You can have a look at it here: [Image:https://copy.com/TQuwy3dwBRl7nEOL] (--[[User:Albedo|Albedo]] ([[User talk:Albedo|talk]]) 21:58, 26 June 2015 (UTC)
...
:::Thank you, Rdm. It’s certainly feasible for small images, although manual editing is a bit tedious, even for this example(binary digits): <https://rosettacode.org/wiki/Binary_digits#.7B.7Bheader.7CPiet.7D.7D>  --[[User:Albedo|Albedo]] ([[User talk:Albedo|talk]]) 15:42, 10 July 2015 (UTC)
:::: Yeah, if you were doing a lot of them, I'd suggest starting with a [[Bitmap]] implementation and then whatever other support you want from [[Raster_graphics_operations]] so you can read an image file. Once you have that, you could use imagemagick to convert to your supported file format. Then build a wiki-table renderer and use that for your Piet code.  And I guess some of us should also start using one of the [piet interpreters](http://www.dangermouse.net/esoteric/piet/tools.html) so we can see your code in action ... --[[User:Rdm|Rdm]] ([[User talk:Rdm|talk]]) 01:47, 11 July 2015 (UTC)
...
:I went to [File list](https://rosettacode.org/wiki/Special:ListFiles) and discovered that all file uploads are blocked since 6/2/2016, even admins have no uploads. Is it sort of technical glitch? Or result of fighting spam too hard?? I'm planning 3 new tasks with 7 pictures. Without picture uploads it is useless. Now this is an old Q (late June): Another interesting issue: Why nobody complains? --AnatolV 19:34, 21 July 2016 (UTC)
