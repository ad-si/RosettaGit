...
I was wondering if the tasks were created in prime holiday time and so got lost, or if they were just uninteresting to the community? I did track down the page for [tasks still in draft mode](https://rosettacode.org/wiki/Category:Draft_Programming_Tasks) and found that there were quite a lot. I decided to add a reference to the draft tasks from the [Category: Programming Tasks](https://rosettacode.org/wiki/Category:Programming_Tasks) page in the hope of getting more people to scan the drafts. Maybe they should be reviewed and pruned (maybe some need shifting to a new category of abandoned tasks)?
...
:: The D entry is a translation of the Python entry, with some refinements. It seems the Python entry gives the same wrong results: <https://ideone.com/ik57yE> And is the C entry having the same problems?
