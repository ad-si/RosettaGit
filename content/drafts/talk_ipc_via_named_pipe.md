...
OpenBSD is also among the last systems to implement threads in userspace. OpenBSD [pthreads(3)](https://www.openbsd.org/cgi-bin/man.cgi?query=pthreads&apropos=0&sektion=3&manpath=OpenBSD+4.9&arch=i386&format=html) is a "user-level library" that [uses non-blocking IO with the kernel](http://archives.zmanda.com/amanda-archives/viewtopic.php?t=4815). We know that <code>open("out", O_WRONLY|O_NONBLOCK)</code> is not possible, so I guess that the library blocks the entire process. This bug should only happen with OpenBSD. Other OS (like Linux, FreeBSD, NetBSD) might have kernel threads.
