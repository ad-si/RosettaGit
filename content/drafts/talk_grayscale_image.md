...
:::::::When something does not give correct results, then it is called incorrect. Which in this case is another way to say that it cannot be added, like apples and oranges. See [pixel](https://en.wikipedia.org/wiki/Pixel) --[[User:Dmitry-kazakov|Dmitry-kazakov]] 17:39, 23 January 2009 (UTC)
...
:::::::::I don't know what did you mean by saying this. An ability to implement certain image operations on images represented using certain color model does not imply that you can do it without any model. The rest is just a lack of some basic understanding of the fundamental difference between representation (computer model) and the modeled thing. Did you have a [CS](https://en.wikipedia.org/wiki/Computer_science) course? In short, you can imagine it as the memory device having states associated with things being modeled, like number, color point, text, employee, program etc. The program itself is merely description of state transitions, see [FSM](https://en.wikipedia.org/wiki/Finite_state_machine). Then you might try to understand the difference between a hardware instruction (merely controlling transitions) and what this instruction is about to implement (like the [ring](https://en.wikipedia.org/wiki/Ring_(mathematics)) operation +). That would help you to learn why there are so many numbers in computing (modular, ranged, saturated, fixed-point, packed decimal, floating-point etc). This might save you many silly discussions in the future. Pixel is not a number in any possible sense. It is not as a model ([for example](https://en.wikipedia.org/wiki/Palette_(computing))), in case you mixed it with what it represents, namely a color point. On the other side a color space is a [vector space](https://en.wikipedia.org/wiki/Vector_space), not a [ring](https://en.wikipedia.org/wiki/Ring_(mathematics)), [field](https://en.wikipedia.org/wiki/Field_(mathematics)) or other mathematical structure associated with vague "number." Furthermore, in the color space the operations on the points can be defined differently depending on semantic meaning of the arguments and the result. For example, when addition of colors means mixing corresponding water-color pigments on white paper as observed by a statistically normal human, or else whatsoever. I hope that helps. --[[User:Dmitry-kazakov|Dmitry-kazakov]] 10:16, 26 January 2009 (UTC)
...
:* [Gamma-Correct Rendering](http://www.sjbrown.co.uk/2004/05/14/gamma-correct-rendering/)
:* [Gamma, Linear Color Space and HDR](http://softimage.wiki.softimage.com/index.php/Gamma,_Linear_Color_Space_and_HDR)
