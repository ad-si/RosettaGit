...
::: There are other problems with ELF shared objects. With systems like [[OpenBSD]], an executable needs some [special code](https://www.openbsd.org/cgi-bin/cvsweb/src/lib/csu/amd64/crt0.c?rev=1.3) to initialize certain variables (like <tt>environ</tt> and <tt>__progname</tt>), to call <tt>main()</tt> and to call <tt>exit()</tt> with the return value from main. A shared object misses this special code, so you cannot start a shared object as an executable. --[[User:Kernigh|Kernigh]] 17:11, 19 April 2011 (UTC)
...
==Too complicated==
This whole thing is getting too complicated. The purpose of ScriptedMain is to list code in as many languages as possible that does this: [https://docs.python.org/library/__main__.html](https://docs.python.org/library/__main__.html) For lack of a better name, it can be called "scripted main". It reinforces the idea that such a function runs when the script is run on its own, not when the module is imported by other code. --[[User:Mcandre]]
