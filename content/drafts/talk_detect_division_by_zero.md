...
:: According to the [wikipedia](https://en.wikipedia.org/wiki/Defined_and_undefined) "In mathematics, defined and undefined are used to explain whether or not expressions have meaningful, sensible, and unambiguous values. Whether an expression has a meaningful value depends on the context of the expression. For example the value of 4 − 5 is undefined if a positive integer result is required."  And the problem with 0/0 is the cardinality of the result, not the lack of any results. --[[User:Rdm|Rdm]] 11:37, 19 June 2010 (UTC)
