...
* Recent Changes feed with more entries than just 25, so that it's easier to keep up to date with ''all'' edits (since any one of them could e.g. be a change to task reqs I need to know about, or a discussion point I want to contribute to, or ...); as it is, new edits often fall off the end of the feed when I check it in the morning. Watching isn't helpful because I would have to watch many pages, and the watch notification mail doesn't include diffs like the feed does. The obvious URL to use is <https://rosettacode.org/mw/index.php?title=Special:RecentChanges&feed=atom&limit=200> , but that still returns only 25. --[[User:Kevin Reid|Kevin Reid]] 11:48, 30 July 2009 (UTC)
