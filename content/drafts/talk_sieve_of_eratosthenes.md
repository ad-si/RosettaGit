...
This task can also be handled by the [NZMATH](http://tnt.math.se.tmu.ac.jp/nzmath/) modules prime and arith1 to enable its generator_eratosthenes function.--[[User:Billymac00|Billymac00]] 02:45, 3 January 2011 (UTC)
...
::BTW wheels can be generated, without using any trial divisions, too (e.g. [this](https://www.haskell.org/haskellwiki/Prime_numbers#Wheeled_list_representation)). -- [[User:WillNess|WillNess]] ([[User talk:WillNess|talk]]) 21:18, 10 September 2014 (UTC)
...
:::::::::: Elibarzilay: (sorry, missed your remark before): the big conceptual difference is that it's using the [tree-like folding](https://en.wikipedia.org/wiki/Fold_%28higher-order_function%29#Linear_vs._tree-like_folds) function [''foldi''](https://en.wikipedia.org/wiki/Fold_(higher-order_function)#Tree-like_folds) instead of the linear folding of ''foldr'', for the logarithmic complexity advantage. M.O'Neill derives Bird sieve's complexity at above n^1.5 (in ''n'' primes produced) but the tree folding code runs empirically at the same orders of growth (~ n^1.2..1.25) as her PQ-based code, which is probably ''n (log n)^2 log(log n)''. Earliest such code that I know of is [Jul 16 2007 post by Dave Bayer](https://www.haskell.org/pipermail/haskell-cafe/2007-July/029077.html). -- [[User:WillNess|WillNess]] ([[User talk:WillNess|talk]]) 21:46, 10 November 2014 (UTC)
...
::::Elibarzilay: good to have a discussion with you. :) About Bird (your <2>) (and it's Richard Bird, not William Byrd): O'Neill actually derives the complexity for it too, and proves it's slightly worse than the optimal TD (your <1>). The trick to making it yet faster (which isn't in the paper) is to fold the composites ''in a tree'', substituting ["foldi" for "foldr"](https://en.wikipedia.org/wiki/Fold_(higher-order_function)#Tree-like_folds). I have a Scheme entry with SICP styled streams, that does that (and of course [[Sieve_of_Eratosthenes#List-based_tree-merging_incremental_sieve|the Haskell entry]] itself). The original version at [haskellwiki](https://www.haskell.org/haskellwiki/Prime_numbers_miscellaneous#Implicit_Heap). 
::::And of course what we should be comparing are [empirical orders of growth](https://en.wikipedia.org/wiki/Analysis_of_algorithms#Empirical_orders_of_growth), not constant factors. So if one is constantly slower than the other by the same factor ''at several size points'', it's fine. TD with postponement (i.e. the optimal TD, your <1>) runs pretty well for lower ranges (10k-20k primes) in Haskell too. It's because its complexity is O(n^1.5/(log n)^0.5) and at low ranges ''log'' is approximated by higher power law coefficient, i.e. its influence can be sizeable (I've commonly seen n^1.3 for low ranges of OTD, going steadily to n^1.45..49 for higher ranges, as it should).
::::About your edits: there's no need to add postponement to Bird's code; it achieves the same effect on its own, because it folds to the right. Postponement achieves the same effect laboriously, and should still be worse computationally [because it folds from the left](https://www.haskell.org/haskellwiki/Prime_numbers#Linear_merging). My version should probably be restored (or maybe you have a tweak on it) because it is a stepping stone from the simple lazy version ''to'' the Bird's version. (It's strange that your "postponed Bird" is slightly faster than the straight version of it.) -- [[User:WillNess|WillNess]] ([[User talk:WillNess|talk]]) 09:17, 14 September 2014 (UTC)
...
why I made [this edit](https://rosettacode.org/mw/index.php?title=Sieve_of_Eratosthenes&diff=201761&oldid=201693):
