...
==New version==
Although there are already several edits to this template it still doesn't work properly for the special cases C# and F#. For example the current version of the C# solution for [Call a function](https://rosettacode.org/mw/index.php?title=Call_a_function&oldid=221098) displays the language name correctly but puts the solution into [[:Category:C Sharp]] which is wrong because it should be put into [[:Category:C sharp]] (s instead of S). 
