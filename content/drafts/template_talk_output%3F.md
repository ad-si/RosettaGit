...
We recently had [some confusion](https://rosettacode.org/mw/index.php?title=Arbitrary-precision_integers_(included)&curid=6036&diff=117617&oldid=117591&rcid=118896) over the meaning of this template. I think it partly comes from the fact that it's a gigantic red box. It makes it look like it's saying the code is incorrect (even though we have {{tmpl|incorrect}} for that). Maybe we could change the color of the box to gray? The wording may need to be reworked too. Maybe it should say something like "This code example shows the desired output when run, but the output is not shown '''on this page'''. Please run the code, copy the output, and paste it with the example."? --[[User:Mwn3d|Mwn3d]] 15:52, 16 August 2011 (UTC)
:Hi, I see what you are getting at and changing to a less severe colour seems good, but even when I try and [explain](https://rosettacode.org/wiki/User_talk:Arm#Output) what is being requested I usually end up with a lot of text. I find it hard to be both explicit and short on this particular task. (But others might do better). :-)
