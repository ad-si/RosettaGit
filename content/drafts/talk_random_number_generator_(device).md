...
: I guess that a solution may use either /dev/random or /dev/urandom. The entropy pool probably has enough entropy for a 32-bit integer. I prefer to use /dev/urandom. Many kernels use a secure hash, like SHA1, to convert the entropy pool to random numbers. ([NetBSD uses SHA1.](http://cvsweb.netbsd.org/bsdweb.cgi/src/sys/dev/rndpool.c?rev=1.20&content-type=text/x-cvsweb-markup&only_with_tag=MAIN)) Even if the entropy pool is dry, the random numbers should remain secure. I guess that you would have to reverse or defeat SHA1 to replicate the pseudorandom numbers from a dry pool; but SHA1 is a secure hash so difficult to reverse.
...
::: Er. I was unclear. Clarification handy, though: [https://irclog.perlgeek.de/rosettacode/2011-01-11#i_3175701](https://irclog.perlgeek.de/rosettacode/2011-01-11#i_3175701) --[[User:Short Circuit|Michael Mol]] 15:21, 11 January 2011 (UTC)
