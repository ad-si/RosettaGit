...
: Digit is not a number, it is a symbol. [See](http://en.wikipedia.org/wiki/Numerical_digit).
...
: Here are some examples of encoding in between bits and bytes: [4-bit character codes](http://en.wikipedia.org/wiki/RADIX-50), [packed decimal numbers](http://en.wikipedia.org/wiki/Binary-coded_decimal).
: This is an example of a serial bit-oriented protocol [CAN](http://en.wikipedia.org/wiki/Controller_Area_Network), note how transmission conflicts are resolved in CAN using the identifier's bits put on the wire. Also note that a CAN controller is responsible to deliver CAN messages to the CPU in the endianness of the later. I.e. it must recode sequences of bits on the wire into 8-bytes data frames + identifiers.
