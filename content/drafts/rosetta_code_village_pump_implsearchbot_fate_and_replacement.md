...
:: Until I have time to set up the new Report namespace, take a look at [these JSON](https://rosettacode.org/json/isb/) files.  There is one JSON file there for every category on the wiki.  Each JSON file contains the contents of the relevant category, with the exception of [this one](https://rosettacode.org/json/isb/_categories.json), which contains the names of all the categories.  There is a running service on the server that updates the JSON files within a few minutes of a page being added to the category. The update is tied to the server's five-second load average, and should almost never take longer than four minutes.  The timestamps on the file, for the most part, reflect the last time the category was updated; The files are only written to if the category contents change between checks, or if the file was deleted by manual means.
...
::: I have hacked together a very crude version (mostly for my own use now) which can be found [in my SVN](http://svn.freebsd.lando.cc/joey/Public/Rosetta%20Code/_ImplSearchBot). This probably won't be of much use to most people here as it only runs locally and it's in PowerShell so it will only run on Windows. Still, maybe someone finds it useful. I think I'll include omit checking tomorrow too. —[[User:Hypftier|Johannes Rössel]] 21:48, 13 September 2009 (UTC)
