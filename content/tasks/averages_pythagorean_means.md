...
[numbers:amean, numbers:gmean, and numbers:hmean](https://www.cs.arizona.edu/icon/library/src/procs/numbers.icn) are shown below:
...
Another implementation using <code>[Array.fold_left](https://caml.inria.fr/pub/docs/manual-ocaml/libref/Array.html#VALfold_left)</code> instead of a '''for''' loop:
...
The command [ameans](https://www.stata.com/help.cgi?ameans) prints the arithmetic, geometric and harmonic means, together with confidence intervals.
<lang>clear all
set obs 10
gen x=_n
ameans x
