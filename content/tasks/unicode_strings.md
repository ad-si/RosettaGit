...
* Extensive support of Unicode (including normalization, collation, etc.) and text codecs are provided by [Matreshka](http://forge.ada-ru.org/matreshka).
...
jq's data types are JSON's data types.  In particular, jq strings are UTF-8 strings. See [json.org](http://json.org) for further details.
