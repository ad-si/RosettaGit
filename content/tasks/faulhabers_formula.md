...
:*   The Wikipedia entry:   [Bernoulli numbers](https://en.wikipedia.org/wiki/Bernoulli_number).
:*   The Wikipedia entry:   [binomial coefficients](https://en.wikipedia.org/wiki/Binomial_coefficient).
...
The following implementation does not use [Bernoulli numbers](https://en.wikipedia.org/wiki/Bernoulli_number), but [Stirling numbers of the second kind](https://en.wikipedia.org/wiki/Stirling_numbers_of_the_second_kind), based on the relation: <math>m^n=\sum_{k=0}^n S_n^k (m)_k=\sum_{k=0}^n S_n^k k!{m\choose k}</math>.
