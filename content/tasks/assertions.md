...
Example given<https://www.xs4all.nl/~jmvdveer/report_4.html#92>:
...
The '[assert](https://software-lab.de/doc/refA.html#assert)' function, in
...
(The current GNU Sather compiler v1.2.3 I am using to test the code seems to ignore the assertion and no fatal error is raised, despite Sather should, see e.g. [here](https://www.gnu.org/software/sather/docs-1.2/tutorial/safety2208.html)).
...
Assertions in Stata are limited to checking a property on the observations of a dataset. See '''[assert](https://www.stata.com/help.cgi?assert)''' in Stata help.
...
There is another command, '''[confirm](https://www.stata.com/help.cgi?confirm)''', that can be used to check existence and type of program arguments or files. For instance, to check that the file titanium.dta exists:
...
It's also possible to use '''[error](https://www.stata.com/help.cgi?error)''' to throw an error if some condition is satisfied. However, this command can only print predefined error messages: it takes the error number as an argument. For instance:
...
Then, if '''[capture](https://www.stata.com/help.cgi?capture)''' is used to trap the error, the return code (here 54) can be retrieved in '''[_rc](https://www.stata.com/help.cgi?_variables)'''.
